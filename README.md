...
为 Luker 添加视频库：上传本地视频、导入视频链接，并将视频附加给 Gemini。

## 功能

- **本地上传**：浏览器直接上传到私有 Google Cloud Storage（GCS）。
- **链接导入**：支持 HTTPS 视频直链，以及 Iwara、B站播放页；可选择处理节点。
- **自动选择存储**：默认不超过 14 MB 的导入视频存放到 copyparty，较大视频上传到 GCS。
- **视频库**：搜索、按存储类型筛选、分页浏览，复制链接或再次附加。
- **可视化设置**：管理员在 Luker 内配置模型、GCS 和导入节点，保存后立即生效。
...
随后启用服务端插件并重启 Luker。管理员打开 **视频库 → 设置**，填写模型连接与 GCS 桶、选择服务账号 JSON，即可保存生效。使用链接导入时，在同一页面添加节点地址和令牌。详见[安装与配置指南](docs/installation.md)。
...
1. 打开附件菜单中的 **视频库：上传与管理**。
2. 在“上传视频”中选择本地文件，或在“链接导入”中粘贴链接并选择节点。
3. 回到“我的视频”，点击**附加**，输入问题并发送。

列表每页显示 12 条；可搜索已加载的视频、筛选 GCS/临时直链。数量较多时点击“加载更多”继续读取。

也可以手动添加 `gs://` 地址或配置允许的 HTTPS 视频直链。视频库支持复制链接和重新附加；文件过期后需要重新上传。
