...
class NamingTest(unittest.TestCase):
    ...
    def test_multilingual_names_fit_filesystem_and_remove_path_characters(self):
        ...
        self.assertLessEqual(len(name.encode()),175);self.assertTrue(name.endswith('.mp4'));self.assertNotIn('/',name)
        self.assertEqual(naming.filename('simple.txt','.txt'),'simple.txt')
    def test_bilibili_prefers_readable_author(self):
        self.assertEqual(naming.webpage_name({'id':'BVtest','title':'Title','uploader':'Author','uploader_id':'123'}),'Author - Title.mp4')
