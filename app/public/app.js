...
async function account(){
    const panel=$('<section>').append($('<h2>').text('插件令牌'));
    ...
    const tokenOutput=$('<div>');panel.append(button('生成专用令牌',async()=>{const result=await api('/tokens',{label:label.input.val(),shared:shared.prop('checked')});tokenOutput.empty().append($('<p>').text('仅显示一次。请在 Luker 的“文件库”入口连接。'),$('<input readonly class="token">').val(result.token));await tokens();}),tokenOutput);
...
async function users(){
    const panel=$('<section>').append($('<h2>').text('用户管理'));
    const currentPassword=field('修改我的独立登录密码（至少 12 字符）','','password');
    panel.append(currentPassword.element,button('更新我的密码',async()=>{await api('/password',{password:currentPassword.input.val()});currentPassword.input.val('');notice('密码已更新。');}));
    if(session.user.admin){
        panel.append($('<h3>').text('其他用户'));
        const list=$('<div>');panel.append(list);
        async function refresh(){list.empty();for(const u of (await api('/users')).items){const row=$('<p>').text(`${u.handle} · ${u.admin?'管理员':'用户'} · ${u.disabled?'停用':'启用'} `);if(u.handle!==session.user.handle)row.append(button(u.disabled?'启用':'停用并撤销全部令牌',async()=>{await api('/users/'+u.handle,{disabled:!u.disabled});await refresh();}));list.append(row);}}
        await refresh();
        const handle=field('新用户名'),password=field('初始密码','','password'),admin=$('<input type="checkbox">');
        panel.append(handle.element,password.element,$('<label>').append(admin,' 管理员'),button('创建用户',async()=>{await api('/users',{handle:handle.input.val(),password:password.input.val(),admin:admin.prop('checked')});password.input.val('');await refresh();}));
    }
    await new Popup(panel,1).show();
}
async function start(){session=await api('/session');csrf=session.csrf;$('#login').hide();$('#account').empty().append($('<span>').text(session.user.handle),button('插件令牌',account),button('用户管理',users));if(session.user.admin)$('#account').append(button('节点与策略',nodes));$('#account').append(button('退出',async()=>{await api('/logout',{});location.reload();}));await showLibrary({API,context,Popup,POPUP_TYPE,POPUP_RESULT,mount:$('#library'),createSettingsPanel:(ctx,saved)=>createSettingsPanel(ctx,saved,API)});}
